...
function dragula (initialContainers, options) {
  ...
  if (o.folderCss === void 0) { o.folderCss = '[type=folder]'; }
  ...
  function drop (item, target) {
    ...
    if (isInitialPlacement(target) && !_isHover) {
      ...
      drake.emit('drop', item, target, _source, _currentSibling, _isHover);
    }
    cleanup();
  }
  ...
  function cleanup () {
    ...
    if (item) {
      ...
      try {
        classes.rm(_currentSibling, 'gu-drop-overlay');
        _isHover && item.remove();
      } catch {}
    }
    if (_renderTimer) {
  ...
  function drag (e) {
    ...
    if (
      ...
      reference !== item && (reference !== nextEl(item) || isFolder(reference))
    ) {
      // console.log(reference)
      if (isFolder(reference) && isHover(reference)) {
        classes.add(reference, 'gu-drop-overlay');
        _isHover = true;
      } else {
        _currentSibling && classes.rm(_currentSibling, 'gu-drop-overlay');
    ...
    function isHover(el) {
      ...
      // debugger
      if (horizontal) {
        return (clientX > rect.left + getRectWidth(rect) / 4) && (clientX < rect.left + getRectWidth(rect) * 3 / 4);
      }
      return (clientY > rect.top + getRectHeight(rect) / 4) && (clientY < rect.top + getRectHeight(rect) * 3 / 4);
    }
  }
